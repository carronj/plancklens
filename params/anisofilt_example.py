"""Parameter file for lensing reconstruction on a masked  curved-sky simulation library.
...
# --- We turn to the inverse-variance filtering library. In this file we use anisotropic filtering,
#     where parts of the sky is masked (we do perform independent T and Pol. filtering)

# Masks (here the planck lensing 2018 mask. Replace this with your mask)
maskpaths = ['/project/projectdirs/cmb/data/planck2018/pr3/Planck_L08_inputs/PR3vJan18_temp_lensingmask_gPR2_70_psPR2_143_COT2_smicadx12_smicapoldx12_psPR2_217_sz.fits.gz']

libdir_cinvt = os.path.join(TEMP, 'cinv_t')
libdir_cinvp = os.path.join(TEMP, 'cinv_p')
libdir_ivfs  = os.path.join(TEMP, 'ivfs')

# Anisotropic filter instance, for temperature and polarization
# Here we use homogeneous noise in the filter outside the masked area
# Feed-in a a inverse noise variance map instead of nlev_t ** 2 to use inhomogeneous noise outside the mask as well
# The factor 3 here is crudely the area of a pixel in arcmin for nside 2048 used in this parameter file
