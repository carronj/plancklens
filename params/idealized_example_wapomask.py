"""Parameter file for lensing reconstruction on a idealized simulation library.
...
apomask_path = os.path.join(TEMP, 'apomask.fits')
if not os.path.exists(apomask_path): # Here we use a crudely apodized version of the Planck 2018 lening mask
    if not os.path.exists(TEMP): os.makedirs(TEMP)
    # FIXME: PATH
    bmask = hp.read_map('/global/cscratch1/sd/jcarron/jpipe/inputs/PR3vApr6_temp_lensingmask_gPR2_70_psPR2_143_COT2_psPR2_217_sz.fits.gz')
    hp.write_map(apomask_path, utils.apodize_mask(bmask, cache_dir=None))
